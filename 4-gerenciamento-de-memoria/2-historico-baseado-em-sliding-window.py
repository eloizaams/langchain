...
def prepare_inputs(payload: dict) -> dict:
    ...
    trimmed = trim_messages(
        raw_history,
        token_counter=len, # Conta a quantidade de mensagens e nao de tokens reais
        max_tokens=2,
        strategy="last", # usa as últimas mensagens enviadas e corta as mais antigas
        start_on="human", # incia a interação com a mensagem do usuário
        include_system=True, # mantém a mensagem do sistema
        allow_partial=False, # não parte as mensagens
    )
...
resp1 = conversational_chain.invoke({"input": "My name is Eloiza. Reply only with 'OK' and do not mention my name."}, config=config)
